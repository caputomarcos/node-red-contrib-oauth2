<script>
	...
	function onClick() {
		...
		if (node.authorizationEndpoint) {
			url = `oauth2/auth?id=${encodeURIComponent(node.id)}&clientId=${encodeURIComponent(
				node.clientId
			)}&clientSecret=${encodeURIComponent(node.clientSecret)}&scope=${encodeURIComponent(
				node.scope
			)}&callback=${encodeURIComponent(node.callback)}&authorizationEndpoint=${encodeURIComponent(
				node.authorizationEndpoint
			)}&redirectUri=${encodeURIComponent(node.redirectUri)}&proxy=${encodeURIComponent(
				node.proxy
			)}`
		} else {
			url = `oauth2/auth?id=${encodeURIComponent(node.id)}&clientId=${encodeURIComponent(
				...
				node.scope
			)}&callback=${encodeURIComponent(node.callback)}&proxy=${encodeURIComponent(node.proxy)}`
		}
