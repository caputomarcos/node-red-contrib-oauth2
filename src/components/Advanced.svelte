...
<Group clazz="paddingAdvanced">
  <Collapsible label="Request" indented={false}>
    ...
    {#if node.clientCredentialsInBody}
      <Callout type="info" small>Ensure that the client credentials are included in the token request body for authentication purposes.</Callout>
    {/if}
    <Input bind:node prop="rejectUnauthorized" labelBeforeCheckbox={true} />
    {#if node.rejectUnauthorized}
      <Callout type="info" small>
        The <b>rejectUnauthorized</b> parameter controls SSL/TLS certificate validation for the server, with true enforcing validation and false disabling it.
      </Callout>
    {/if}
    <Input bind:node prop="keepAuth" labelBeforeCheckbox={true} />
    {#if node.keepAuth}
      <Callout type="info" small>msg.openApiToken and msg.headers will not be deleted and can be seen by other nodes in the flow.</Callout>
    {/if}
  </Collapsible>
</Group>
